...
![image-20251229122912910](assets/3693258-20251229122916937-1228340574.png)

### 1.1数据集的预处理

假设学生将会得到 *y* 分在最后的期末测试中，同时假设该分数和学生平时花费在该课程上面的时间 *x* 相关，那么根据下面这个表格，可以提问，当学生花费时间为 *4* 小时的时候他们的期末测试分数大概为多少分。

![image-20251229123843651](assets/3693258-20251229123854650-1957288951.png)

就像上面说到的，在期末考试中已经得到了很多个数据，这些数据是学习时间和最后取得分的关系，求解并推理当$x=4$的时候这个分数大概是多少$y=?$。

![image-20251229124306988](assets/3693258-20251229124309176-1517187077.png)
...
![image-20251229124634171](assets/3693258-20251229124639213-119885691.png)
...
![image-20251229124836849](assets/3693258-20251229130544615-207389534.png)

### 1.2 模型的引入

在这里所谓的模型就是想要找到一个比较好的函数$y =  f(x)$，能够比较完美的适合这个输入于输出。

![image-20251229125128548](assets/3693258-20251229125131749-769583247.png)

现在我们就使用一个线性模型来进行举例。

> y上面有一个小帽子表示的是我们自己的预测模型

![image-20251229125354466](assets/3693258-20251229125357299-1908660569.png)

我们要求的就是两个参数$w , b$

现在我们给出咱们的输入参数和输出的**训练集模型**。

![image-20251229123843651](assets/3693258-20251229123854650-1957288951-1774611538133-95.png)
...
![image-20251229130009074](assets/3693258-20251229130011921-495989726.png)

`MSE`平均平方误差。

这个时候可以根据这样的公式进行损失上面的一个计算。

![image-20251229130421723](assets/3693258-20251229130423683-908192984.png)
...
![image-20250826134117257](assets/3693258-20250908090300870-2115907160.png)
...
![image-20250826141422475](assets/3693258-20250908090305544-1336740656.png)
